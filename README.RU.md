...
Это мой форк библиотеки [LedControl](https://github.com/wayoda/LedControl) Я полностью переработал исходную библиотеку. Целю было получить более гибкий интерфейс, оптимизировать читаемость кода и производительность.

В настоящее время библиотека полностью готова к использованию.
...
С помощью библиотеки `LedMAtrix` вы можете гибко управлять светодиодными матрицами и каскадами таких матриц. Она может работать как через программный SPI используя для подключения любые три свободные пина, так и через аппаратный интерфейс SPI. 
...
Как вы успели заметить, используется библиотека `Pino`. Конструкторы легко принимают в качестве аргументов целые числа.
...
void wakeup() const;
```
Метод `wakeup()` велючает питание матрицы, если оно было ранее выключено.
...
То есть в место неуклюжего кода
```c
uint8_t foo(/*...*/) {/*...*/}
